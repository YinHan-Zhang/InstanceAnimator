...
def get_video_to_video_latent(input_video_path, track_video_path, video_length, sample_size, fps=None, validation_video_mask=None, ref_image=None):
    ...
    #     if isinstance(track_video_path, str):
    #         cap = cv2.VideoCapture(track_video_path)
    #         track_video = []
    ...
    #             if frame_count % frame_skip == 0:
    #                 frame = cv2.resize(frame, (sample_size[1], sample_size[0]))
    #                 track_video.append(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))

    #             frame_count += 1

    #         cap.release()
    #     else:
    #         track_video = track_video_path

    #     track_video = torch.from_numpy(np.array(track_video))[:video_length]
    #     track_video = track_video.permute([0, 3, 1, 2]).unsqueeze(0) / 255 # B, T, C, H, W
