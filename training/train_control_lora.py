...
from src.data.bucket_sampler import (ASPECT_RATIO_512,
                                            ASPECT_RATIO_RANDOM_CROP_512,
                                            ASPECT_RATIO_RANDOM_CROP_PROB,
                                            AspectRatioBatchImageVideoSampler,
                                            RandomSampler, get_closest_ratio)
from src.data.dataset_image_video import (ImageVideoControlDataset,
                                                 ImageVideoDataset,
                                                 ImageVideoSampler,
                                                 get_random_mask,
                                                 process_pose_file,
                                                 process_pose_params)
from src.models import (AutoencoderKLWan, CLIPModel, WanT5EncoderModel,
                               WanTransformer3DModel)
from src.pipeline import WanFunControlPipeline
from src.utils.discrete_sampler import DiscreteSampling
from src.utils.lora_utils import (create_network, merge_lora,
                                         unmerge_lora)
from src.utils.utils import (get_image_to_video_latent,
                                    get_video_to_video_latent,
                                    save_videos_grid)

from co_tracker.cotracker.utils.visualizer import Visualizer , read_video_from_path
from co_tracker.cotracker.utils.new_visualizer import NormalizedFirstFrameVisualizer 
from co_tracker.cotracker.predictor import CoTrackerPredictor

from dinov2.dino_model import FrozenDinoV2Encoder
...
def main():
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            new_examples["video_path"]         = []
            new_examples["text"]         = []
            # Used in Control Mode
            new_examples["control_pixel_values"] = []
            # new_examples["track_pixel_values"] = []
            new_examples["bg_images"] = []
            ...
            for example in examples:
                ...
                new_bg_image = torch.from_numpy(example["bg_images"]).permute(2, 0, 1).contiguous()
                bg_image = new_bg_image / 255.

                _, channel, h, w = pixel_values.size()
                num_subject = len(example["subject_images"])
                new_subject_image = torch.zeros(num_subject, channel, h, w)
                if num_subject != 0:
                    subject_image = torch.from_numpy(example["subject_images"]).permute(0, 3, 1, 2).contiguous()
                ...
                new_examples["bg_images"].append(transform(bg_image))
                ...
                new_examples["video_path"].append(example["video_path"])
                ...
                if args.train_mode != "control":
                    ...
                    # ref_pixel_values = new_examples["pixel_values"][-1][clip_index].unsqueeze(0) # 首帧
                    ref_pixel_values = new_examples["bg_images"][0].unsqueeze(0) # 背景
            ...
            new_examples["bg_images"] = torch.stack([example[:batch_video_length] for example in new_examples["bg_images"]])
            min_ins = min([len(example) for example in new_examples["subject_images"]])
            new_examples["subject_images"] = torch.stack([example[:min_ins] for example in new_examples["subject_images"]])
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                if args.train_mode != "control":
                    ref_pixel_values = batch["ref_pixel_values"].to(weight_dtype) # bg
                ...
                with torch.no_grad():
                    ...
                    if args.train_mode != "control":
                        ...
                        ref_latents_conv_in = ref_latents.repeat(1,1,latents.shape[2],1,1)

                        # instances latent
                        batch["subject_images"] = batch["subject_images"].to(weight_dtype)
                        instances_latent = _batch_encode_vae(batch["subject_images"])

                        if control_latents is None:
                            control_latents = ref_latents_conv_in
                        else:
                            control_latents = torch.cat([control_latents, instances_latent], dim = 2)
                            ref_latents_conv_in = torch.cat([ref_latents_conv_in, instances_latent], dim = 2)
                            control_latents = torch.cat([control_latents, ref_latents_conv_in], dim = 1)

                        # background clip feature
                        clip_context = []
                        ...
                        # instance clip feature
                        for images in batch["subject_images"][0]:
                            images = images.permute(1,2,0)
                            clip_image = Image.fromarray(np.uint8(images.float().cpu().numpy()))
                            clip_image = TF.to_tensor(clip_image).sub_(0.5).div_(0.5).to(clip_image_encoder.device, weight_dtype)
                            _clip_context = clip_image_encoder([clip_image[:, None, :, :]])

                            clip_context = torch.cat([clip_context, _clip_context], dim=1)
                ...
                num_frames = num_frames + instances_latent.shape[2] # add ref instances
                ...
                noisy_latents = torch.cat([noisy_latents, instances_latent], dim = 2)
                ...
                with torch.cuda.amp.autocast(dtype=weight_dtype), torch.cuda.device(device=accelerator.device):
                    noise_pred = transformer3d(
                        ...
                        subject_ref=instances_latent,
                    )

                noise_pred = noise_pred[:,:,:-instances_latent.shape[2]]
                ...
                if global_step % args.checkpointing_steps == 0:
                    if args.use_deepspeed or args.use_fsdp or accelerator.is_main_process:
                        ...
                        if not args.save_state:
                            ...
                            logger.info(f"Saved lora to {safetensor_save_path}")
