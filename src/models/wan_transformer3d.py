...
class WanI2VCrossAttention(WanSelfAttention):
    ...
    def forward(self, x, context, context_lens, dtype=torch.bfloat16, t=0, ins_num=1, text_control_strength=None, bg_control_strength=None, ins_control_strength=None):
        ...
        Args:
            ...
            context_lens(Tensor): Shape [B]
        """
        context_img = context[:, :257,:] # bg
        context_ins = context[:, 257:257*ins_num] # ins
        
        context = context[:, 257*ins_num:]
        b, n, d = x.size(0), self.num_heads, self.head_dim
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        def cross_attn_ffn(x, context, context_lens, e):
            # cross-attention
            x = x + self.cross_attn(
                self.norm3(x), context, context_lens, dtype, t=t, 
                ins_num=ins_num,
                text_control_strength=None, 
                bg_control_strength=None, 
                ins_control_strength=None
...
class WanTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        self.instance_embedding = nn.Conv3d(
    ...
    def disable_riflex(self):
        ...
        self.freqs = torch.cat(
            [
                rope_params(1024, self.d - 4 * (self.d // 6)), # 时间维度,总维度减去空间维度总和
                rope_params(1024, 2 * (self.d // 6)), # 高度维度
                rope_params(1024, 2 * (self.d // 6)) # 宽度维度
            ],
            dim=1
        ).to(device)
    ...
    def forward(
        ...
        if subject_ref is not None:
            ...
            subject_ref = self.instance_embedding(subject_ref).flatten(2).transpose(1, 2)
        ...
        if clip_fea is not None:
            context_clips = []
            for clip_img in clip_fea:
                ins_num = int(clip_img.shape[1] // 257)
                context_clip = self.img_emb(clip_img)  # bs x 257 x dim
                context_clips.append(context_clip.unsqueeze(0))
            
            context_clips = torch.cat(context_clips, dim=0)
            context = torch.concat([context_clips, context], dim=1)
    ...
    @classmethod
    def from_pretrained(
        cls, pretrained_model_path, transformer_path=None, subfolder=None, transformer_additional_kwargs={},
