        ...
        if data_info.get('type', 'image') =='video':
            ...
            with VideoReader_contextmanager(video_dir, num_threads=2) as video_reader:
                ...
                try:
                    ...
                    raise ValueError(f"Failed to extract frames from original video. Error is {e}.")
            ...
            # sketch video
            control_video_id = data_info['sketch_file_path']
                ...
                if control_video_id is not None:
                    with VideoReader_contextmanager(control_video_id, num_threads=2) as control_video_reader:
                        try:
                            ...
                            raise ValueError(f"Failed to extract frames from sketch video. {control_video_id}. Error is {e}.")
            ...
            if self.enable_subject_info:
                ...
                subject_id = data_info.get('control_file_path', [])[:6]
                shuffle(subject_id)
                subject_images = []
            ...
            # background 
            bg_image = Image.open(data_info['background_path'])
            width, height = bg_image.size
            total_pixels = width * height
            bg_image = padding_image(bg_image, visual_width, visual_height)
            if random.random() < 0.5:
                bg_image = bg_image.transpose(Image.FLIP_LEFT_RIGHT)
            bg_image = np.array(bg_image)

            video_path = data_info['sketch_file_path']

            return pixel_values, control_pixel_values, subject_image, bg_image, control_camera_values, text, video_path, "video"
        else:
            image_path, text = data_info['file_path'], data_info['text']
            if self.data_root is not None:
                image_path = os.path.join(self.data_root, image_path)
            ...
            control_image_id = data_info['sketch_file_path']
            ...
            if self.enable_subject_info:
                ...
                subject_id = data_info.get('control_file_path', [])
                
                shuffle(subject_id)
                subject_images = []
                for i in range(min(len(subject_id), 6)):
                    subject_image = Image.open(subject_id[i])
                    width, height = subject_image.size
                    total_pixels = width * height
            ...
            return image, control_image, subject_image, canvas_image, None, text, video_path, 'image'
    def __len__(self):
        return self.length
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            try:
                ...
                pixel_values, control_pixel_values, subject_image, bg_image, control_camera_values, name, video_path, data_type = self.get_batch(idx)

                sample["pixel_values"] = pixel_values
                sample["control_pixel_values"] = control_pixel_values
                sample["subject_images"] = subject_image
                sample["bg_images"] = bg_image
                sample["text"] = name
