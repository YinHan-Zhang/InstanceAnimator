...
class WanPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        shift: int = 5,
        text_control_strength=None,
        bg_control_strength=None, 
        ins_control_strength=None  
        ...
        if control_video_latents.shape[2] > latents.shape[2]:
            control_video_latents = control_video_latents[:,:,:latents.shape[2]]
        elif control_video_latents.shape[2] < latents.shape[2]:
            padding = int(latents.shape[2] - control_video_latents.shape[2])
            pad = torch.zeros((1, latents.shape[1], padding, latents.shape[3], latents.shape[4])).to(device, weight_dtype)
            control_video_latents = torch.cat([control_video_latents, pad], dim=2).to(device, weight_dtype)

        if bg_image is not None:
            video_length = bg_image.shape[2]
            bg_image = self.image_processor.preprocess(rearrange(bg_image, "b c f h w -> (b f) c h w"), height=height, width=width) 
            bg_image = bg_image.to(dtype=torch.float32)
            bg_image = rearrange(bg_image, "(b f) c h w -> b c f h w", f=video_length)
            
            bg_image_latentes = self.prepare_control_latents(
                None,
                bg_image,
                batch_size,
                height,
            ...
            start_image_latentes_conv_in = bg_image_latentes.repeat(1,1,latents.shape[2],1,1)
        

        # Prepare clip latent variables
        if clip_image is not None:
            clip_context = []
            for img in clip_image:
                img = TF.to_tensor(img).sub_(0.5).div_(0.5).to(device, weight_dtype) 
                img_context = self.clip_image_encoder([img[:, None, :, :]])
                clip_context.append(img_context)
            clip_context = torch.cat(clip_context, dim=1)
        else:
        ...
        target_shape = (self.vae.latent_channels, ((num_frames - 1) // self.vae.temporal_compression_ratio + 1)+len(subject_ref_images) , width // self.vae.spatial_compression_ratio, height // self.vae.spatial_compression_ratio)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                subject_ref_ins = []
                for ref_ins in subject_ref_images:
                    ref_ins = self.image_processor.preprocess(rearrange(ref_ins, "b c f h w -> (b f) c h w"), height=height, width=width) 
                    ref_ins = ref_ins.to(dtype=torch.float32)
                    ref_ins = rearrange(ref_ins, "(b f) c h w -> b c f h w", f=1)
                    ref_ins_latentes = self.prepare_control_latents(
                        None,
                        ref_ins,
                        batch_size,
                        height,
                    ...
                    subject_ref_ins.append(ref_ins_latentes.to(device, weight_dtype))
                
                subject_ref_ins = torch.cat(subject_ref_ins, dim=2)

                subject_latents_input = (
                        torch.cat([subject_ref_ins] * 2) if do_classifier_free_guidance else subject_ref_ins
                    ).to(device, weight_dtype)

                
                latent_model_input = torch.cat([latent_model_input, subject_latents_input], dim=2)
                control_latents_input = torch.cat([control_latents_input, subject_latents_input], dim=2)
                start_image_latentes_conv_in_input = torch.cat([start_image_latentes_conv_in_input, subject_latents_input], dim=2)

                control_latents_input = torch.cat([control_latents_input, start_image_latentes_conv_in_input], dim = 1)
                ...
                noise_pred = noise_pred[:,:,:-len(subject_ref_images)]
